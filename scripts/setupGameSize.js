...
commander
  .option(
    "--width <val>",
    "Base width of your game (Default: " + defaultWidth + ")",
    function(val) {
  ...
  .option(
    "--height <val>",
    "Base height of your game (Default: " + defaultHeight + ")",
    function(val) {
      return parseInt(val, 10);
    },
    -1
  ...
  .option(
    "--aspect-ratio <val>",
    "Aspect ratio of your game (Default: " + defaultAspectRatio + ")",
    function(val) {
  ...
  .option(
    "--scale-mode <val>",
    "Scale mode of your game (Default: " +
      defaultHeight +
      ") [USER_SCALE, SHOW_ALL, EXACT_FIT, NO_SCALE, RESIZE]",
    function(val) {
...
if (scaleMode === null) {
  console.log(
    "--scale-mode must be one of the available options; [USER_SCALE, SHOW_ALL, EXACT_FIT, NO_SCALE, RESIZE]"
  );
  return;
}
...
var greenColor = { red: 0, green: 125, blue: 0, alpha: 100 };
...
if (scaleMode === "USER_SCALE") {
  ...
  if (providedAspectRatio !== aspectRatio) {
    throw "Base width and height must result in a " +
      aspectRatio +
  ...
  image.fillRect(
    maxWidthDifferenceHalf,
    maxHeightDifferenceHalf,
    baseWidth,
    baseHeight,
    greenColor
  );
  ...
  image.fillRect(
    maxWidthDifferenceHalf + baseWidth,
    maxHeightDifferenceHalf,
    maxWidthDifferenceHalf,
    baseHeight,
    blueColor
  ...
  image.fillRect(
    maxWidthDifferenceHalf,
    maxHeightDifferenceHalf - midHeightDifferenceHalf,
    baseWidth,
    midHeightDifferenceHalf,
    yellowColor
  );
  image.fillRect(
    maxWidthDifferenceHalf,
    maxHeightDifferenceHalf + baseHeight,
    baseWidth,
    midHeightDifferenceHalf,
    yellowColor
  );
  ...
  image.fillRect(
    maxWidthDifferenceHalf,
    maxHeightDifferenceHalf + baseHeight + midHeightDifferenceHalf,
    baseWidth,
    maxHeightDifferenceHalf,
    redColor
  );
...
shell.ls("webpack.*.config.js").forEach(function(file) {
  shell.sed(
    "-i",
    /\/\*\[\[DEFAULT_GAME_WIDTH\*\/\S*\/\*DEFAULT_GAME_WIDTH\]\]\*\//,
    "/*[[DEFAULT_GAME_WIDTH*/" + baseWidth + "/*DEFAULT_GAME_WIDTH]]*/",
    file
  );
  shell.sed(
    "-i",
    /\/\*\[\[DEFAULT_GAME_HEIGHT\*\/\S*\/\*DEFAULT_GAME_HEIGHT\]\]\*\//,
    "/*[[DEFAULT_GAME_HEIGHT*/" + baseHeight + "/*DEFAULT_GAME_HEIGHT]]*/",
    file
  );
  shell.sed(
    "-i",
    /\/\*\[\[MAX_GAME_WIDTH\*\/\S*\/\*MAX_GAME_WIDTH\]\]\*\//,
    "/*[[MAX_GAME_WIDTH*/" + maxWidth + "/*MAX_GAME_WIDTH]]*/",
    file
  );
  shell.sed(
    "-i",
    /\/\*\[\[MAX_GAME_HEIGHT\*\/\S*\/\*MAX_GAME_HEIGHT\]\]\*\//,
    "/*[[MAX_GAME_HEIGHT*/" + maxHeight + "/*MAX_GAME_HEIGHT]]*/",
    file
  );
  shell.sed(
    "-i",
